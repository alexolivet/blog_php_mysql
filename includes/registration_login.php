...
if (isset($_POST['reg_user'])) {
	...
	if (count($errors) == 0) {
 	...
 	if ( in_array($_SESSION['user']['role'], ["Admin"])) {
 		...
 		$_SESSION['message'] = "You are now logged in";
			// redirect to public area or destroy session
 		//session_destroy();
	...
	if (empty($errors)) {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			if ( in_array($_SESSION['user']['role'], ["Admin"])) {
				...
				//header('location: index.php');		
				//header('location: ' . BASE_URL . '/admin/create_post_author.php');			
				header('location: ' . BASE_URL . 'admin/users_update_author.php?edit-admin=' . $reg_user_id  );			
				exit(0);
			}
		} else {
